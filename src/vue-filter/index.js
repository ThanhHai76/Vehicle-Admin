...
import { convert_price_to_text } from '@/thirdParty/n2vi'
import { number_to_text } from '@/thirdParty/convert_money_string'
...
Vue.filter('fomatPriceWord', data => {
  if (!data) return
  return convert_price_to_text(data)
})

Vue.filter('formatPriceToText', data => {
  if (!data) return 'không'
  return number_to_text(data)
})
...
Vue.filter('transformNumber', (data) => {
  return data ? data.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.') : data
})
